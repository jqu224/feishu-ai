...
- **提神活动**：100 张元气图形（25 手工马赛克 + 75 确定性渐变像素画，⬜ 浅色画布），点一下就换。
- **解压**：6 个文字互动练习——方块呼吸 4-4-4-4、4-7-8 呼吸、54321 五感着陆、正念数息、身体扫描、握拳放松；呼吸与绷紧/松开段落由服务器节拍器每秒推卡自动倒计时（只需点一次「开始」，进行中可「停止」），54321 / 数息 / 扫描按自己的节奏点下一步，完成有庆祝。
...
- 发 `玩法` 或任意未知指令 → 玩法中心主页，所有玩法点按钮进入（也可以直接发 `答题` / `测试` / `猜词` / `剧情` / `变形球` / `解压`）。
...
src/
  ...
  actions/calm.js   解压（点一下走一步的文字互动练习：呼吸 / 54321 / 冥想）
  actions/fact.js   抽条冷知识
  actions/rps.js    猜拳
  actions/vote.js   投票
  actions/form.js   报名表
data/               预制内容：trivia 题库 / tests 题库 / words 词库 / stories 剧情 / facts 冷知识 / ball 帧 / refresh 图形 / calm 练习
