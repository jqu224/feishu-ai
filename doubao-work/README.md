...
| 技能 | 用途 | 门槛 |
|---|---|---|
| **[`XsiaoLung-doc-qa`](skills/XsiaoLung-doc-qa/)**（主推） | 公开飞书文档 → 问答知识库：抓取、分块、建索引、带出处回答、基于文档出题 | **零门槛**：导入 Skill + 一个公开链接，纯内置能力 |
| [`XsiaoLung-card`](skills/XsiaoLung-card/)（进阶） | 飞书群里发起可交互投票/报名/表单卡片 | 需 MCP 连接器 + 主 BOT，适合动手党 |

## 快速开始（30 秒上手）

### 1. 导入技能

豆包电脑版侧边栏 → **「技能 · 连接器 · 伙伴」** → 技能 → 新建/导入本地技能文件 → 选择 `skills/XsiaoLung-doc-qa/`（SKILL.md 会带出 references/）。也可以在豆包工作中用「技能」搜索关键词 `文档问答 / 知识库` 自动匹配。
...
每条回答都带出处（哪个章节），还能基于文档出题做考核。一次真实运行示例见 [`XsiaoLung-doc-qa/references/example-run.md`](skills/XsiaoLung-doc-qa/references/example-run.md)。

## 进阶（可选）：交互卡片 + MCP 连接器

想让豆包工作直接在飞书群里发**可点击的投票/报名卡片**（点击即投、原地统计），才需要走到这一层。`XsiaoLung-card` 技能 + `src/` MCP 连接器（5 个工具）负责这件事，依赖主 BOT（`../chatbot/`）的控制 API。完整接法见 **[docs/setup-doubao-work.md](docs/setup-doubao-work.md)**。
...
飞书群 ──长连接──► chatbot/（Card v2 · 卡片交互回调）
                         ...
                         │
豆包工作 ──MCP──► doubao-work/（连接器 + XsiaoLung-card 技能）
```
...
打包产物：`dist/XsiaoLung-doc-qa.zip`（零门槛技能）· `dist/XsiaoLung-card.zip`（进阶技能）。
...
doubao-work/
├── skills/
│   ├── XsiaoLung-doc-qa/          # 【主推】文档问答知识库：SKILL.md + references/（示例运行 + 排查）
│   └── XsiaoLung-card/            # 【进阶】飞书群交互卡片：SKILL.md + references/examples.md
